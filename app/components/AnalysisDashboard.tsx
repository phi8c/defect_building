...
export const AnalysisDashboard: React.FC = () => {
  ...
  const [detectorType, setDetectorType] = useState<"yolo" | "seg">("yolo");
  ...
  const handleAnalyze = async () => {
    ...
    setError(null);
    try {
      const analysis = await analyzeDefectImage(image, detectorType);
      setResult(analysis);
    } catch (err: any) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full">
      ...
      <div className="lg:col-span-7 flex flex-col h-full">
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex-1 flex flex-col overflow-visible">
  ...
  <select
    value={detectorType}
    onChange={(e) =>
      setDetectorType(e.target.value as "yolo" | "seg")
    }
    className="text-xs border border-slate-200 rounded-md px-2 py-1 bg-white w-[80px] sm:w-[140px]"
  >
          ...
          <div className="flex-1 p-6 overflow-y-auto">
            {!result && !error && !isAnalyzing && (
              <div className="h-full flex flex-col items-center justify-center text-center text-slate-400">
                ...
                <p className="text-sm">
                  Upload an image and run recognition to see results.
                </p>
              </div>
            )}
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-xs font-bold uppercase border ${getSeverityColor(result.severity)}`}
                    >
